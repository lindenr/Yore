...
void ev_mthrow (MonsID mons, ItemID item, int ydest, int xdest)
{
	...
	union ItemLoc loc = it_monsfloc (mons, speed);
	int d, z, y, x;
	mons_getloc (mons, &d, &z, &y, &x);
	bres_init (&loc.fl.bres, y, x, ydest, xdest);
	item_put (item, loc);
	ev_queue (60, proj_move, item);
}

void ev_proj_move (ItemID item)
{
	...
	if (fl.speed <= 0)
	{
		ev_queue (0, proj_done, item);
		return;
	}
	bres_iter (&fl.bres);
	struct DLevel *lvl = dlv_lvl (fl.dlevel);
	if (!map_passable (lvl, fl.zloc, fl.bres.cy, fl.bres.cx))
	{
		ev_queue (0, proj_hit_barrier, item);
		return;
	}
	fl.yloc = fl.bres.cy;
	fl.xloc = fl.bres.cx;
	...
	MonsID mons = lvl->monsIDs[dlv_index (lvl, fl.zloc, fl.yloc, fl.xloc)];
	if (mons)
		ev_queue (0, proj_hit_monster, item, mons);
	ev_queue (60, proj_move, item);
	ev_should_refresh = 1;
}

void ev_proj_done (ItemID item)
{
	if (it_fragile (item))
	{
...
void ev_proj_hit_barrier (ItemID item)
{
	if (!it_persistent (item))
	{
...
void ev_proj_hit_monster (ItemID item, MonsID mons)
{
	struct ItemInFlight fl;
	if (!it_flight (item, &fl))
		return;
	mons_anger (fl.frID, mons);
	...
	mons_take_damage (mons, fl.frID, damage, it_dtyp (item));
	fl.speed = 0;
	item_put (item, (union ItemLoc) {.fl = fl});
	if (!fl.frID)
		return;
...
void ev_mdoshield (MonsID mons, int ydir, int xdir)
{
	...
	// TODO status
	//mons->status.defending.ydir = ydir;
	//mons->status.defending.xdir = xdir;
}
...
void ev_mdohit (MonsID fr, int arm, int zdir, int ydir, int xdir)
{
	...
	if (!mons_take_damage (to, fr, damage, it_dtyp (with)))
		return;
	if (with && (it_flag (with, ITF_FIRE_EFFECT)))//(it_ityp (with)->flags & ITF_FIRE_EFFECT))
	{
		int i, j;
...
{
	if (!mons_bleeding (mons))
		return; // TODO transparently check via event code gen
	int damage = rn(5);
...
{
	//if (mons_attacking (mons) == arm)
	//	return; // should be captured by code gen
	if (mons_getweap (mons, arm))
		mons_unwield (mons, arm);
	//TODO check still valid
	// maybe do in ev_queue in code gen?
...
{
	// mons_takeoff_armour (mons, item);
	// return mons_poll (mons);
	/*if (it_invID(item) != mons ||
		(!it_worn(item)))
		return;*/
...
void ev_mdrop (MonsID mons, V_ItemID items)
{
	...
	for (i = 0; i < items->len; ++ i)
	{
		...
		if (it_worn (drop))
			continue;
		item_put (drop, (union ItemLoc) { .dlvl = {LOC_DLVL, d, z, y, x}});
	}
	v_free (items);
}
...
void ev_mfireball (MonsID mons, int ydest, int xdest, int attk)
{
	...
	union ItemLoc loc = it_monsfloc (mons, 10); //mons->str); // TODO
	bres_init (&loc.fl.bres, y, x, ydest, xdest);
...
void ev_mwater_bolt (MonsID mons, int ydest, int xdest, int attk)
{
	...
	union ItemLoc loc = it_monsfloc (mons, 10); //mons->str);
	bres_init (&loc.fl.bres, y, x, ydest, xdest);
...
void ev_mflash (MonsID mons, int speed, Tick duration)
{
	...
	//	return;
	//mons->status.flashing.end = curtick + duration;
	//mons->status.flashing.speed = speed;
