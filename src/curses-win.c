...
void write_con(uint32_t * n, char *s, uint32_t len)
{
	...
	{
		if (colSan(GET_COLOUR_ATTR(n[i])) == colSan(col))
			continue;
...
char getch()
{
  ...
  try_again:
	...
	else if (k.bKeyDown == TRUE && 0x24 < k.wVirtualKeyCode
			 && k.wVirtualKeyCode < 0x29)
