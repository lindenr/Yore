...
const int ACTUAL_COLOURS[8] = { 1, 31, 32, 34, 41, 42, 44, 4 };	/* Numbers for 
																   ^[[X;Y;Zm. */
...
{
	if (!w)
		return GET_COLOUR_ATTR(COL_TXT_DEF);
	if (w == (uint32_t) - 1)
		return 1;
	return w;
}
...
void write_con(uint32_t * n, char *s, uint32_t len)
{
	...
	for (i = 0; s[i]; ++i)
	{
		...
		printf("%c[0m", CHR_ESC);
		if (u & 1)
			printf("%c[%dm", CHR_ESC, ACTUAL_COLOURS[0]);
		u >>= 1;
		if (u & 7)
			printf("%c[%dm", CHR_ESC, 30 + (u & 7));
		u >>= 3;
		if (u & 8)
			printf("%c[%dm", CHR_ESC, ACTUAL_COLOURS[7]);
		printf("%c", s[i]);
	}
}
...
void refresh()
{
	...
	for (y = 0; y < 25; ++y)
	{
		...
		for (x = 0; x < 80; ++x)
		{
			...
			if (firstc != -1u)
				line[x - firstc] = Current_buffer[ybuf + x];
		}
		if (firstc == -1u)
			firstc = 0;
		if (lastc)
